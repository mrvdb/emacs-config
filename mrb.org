...
* Emacs configuration                                              [3/96]                      :emacs:exclude:
:PROPERTIES:
:CATEGORY: Emacs
:END:
...
I installed the package fill-column-indicator which can draw a
vertical line on the fill column (or any other column for that
matter).

This line may come in handy when doing sensitive non-auto-fill work
(like python coding). I'm not enabling it by default for any mode, but
enabling the loading of the package.
    ...
    (setq mrb/packages
          (append
           ...
           '(smex apache-mode gnuplot-mode sudo-save
                  ...
                  adaptive-wrap elpy ggtags fill-column-indicator)
