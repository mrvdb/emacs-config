...
I want to use the gnome keyring as much as possible, so let's
configure that in as the sole source and see how far we float. We
define two things:
1. that our auth-sources include the login secrets;
2. we encrypt with the key specified by marcel@hsdev.com

#+BEGIN_SRC emacs-lisp
(setq auth-sources (quote ("secrets:Login"))) ;; Not sure what this does, seems to have no effect?
(setq epa-file-encrypt-to "marcel@hsdev.com")
#+END_SRC
  ...
  (setq mrb/packages
        (append
         ...
         '(smex ace-jump-mode apache-mode gnuplot-mode sudo-save erc-highlight-nicknames
                edit-server magit markdown-mode rainbow-delimiters scratch popwin
                highlight-parentheses multiple-cursors wrap-region git-commit-mode git-modes cl-lib keywiz
                smart-tab)
...
#+BEGIN_SRC emacs-lisp
    ...
    (defun run-client-settings()
      ...
      (defvar mrb-bg-highlight "gray22")
...
#+BEGIN_SRC emacs-lisp
  (setq
   org2blog/wp-track-posts (list (concat org-directory "blogs/blogs.org") "To be filed properly")
  )
#+END_SRC
...
Typically in org-mode, after writing a subtree, which is one blog
post in this case, a call to =org-jekyll-export-current-entry= is
called. This will try to deduce which project org-mode must use to
publish the entry with based on the filename, i.e. is the file in or below the
':base-directory' of one the publishing projects and has the proper
extension and so on.
...
#+BEGIN_SRC emacs-lisp
  ...
  (setq org-publish-project-alist
    ...
    ("mrblog-org"
            ...
            :base-directory "~/.outlet/"
            :base-extension "org"

            ;; Path to your Jekyll project.
            :publishing-directory "~/.outlet/blogs/mrblog/"
            :recursive t
      ...
      ("mrblog-files"
            ...
            :publishing-directory "~/.outlet/blogs/mrblog/files/"
            :recursive t
