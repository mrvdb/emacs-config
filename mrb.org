...
Last update: <2022-01-09 11:00:10>
...
The config file =mrb.org= gets opened *a lot* because I tend to fiddle with this config at least once a day. So, it warrants it's own keybinding.
...
#+begin_src sh :exports code :tangle ~/bin/edit :shebang #!/bin/bash
  ...
  # is hard-coded to exec emacs --daemon, sigh
...
#+begin_src emacs-lisp
  ...
  (setq use-package-always-ensure nil      ; Make sure this is nil, so we do not use package.el
        use-package-always-demand t        ; FIXME: only when using daemon?
        )
...
#+begin_src emacs-lisp
  ...
  ;; This is save because emacs tracks if the file is saved in the editing buffer
...
Integrate this with orgmode as well. This is, for now, effectively making a link type available so we can directly link, very efficiently, to pages in PDF documents.
...
#+begin_src emacs-lisp
  ...
  ;; Give commands the option to display full-screen (so far, magit-status only)
...
#+begin_src emacs-lisp
  (use-package org
    :straight (:branch "release_9.5"
               :includes (org-capture org-crypt org-datetree org-protocol))
    :mode ("\\.txt\\'" "\\.org\'")
...
#+end_src
*** Calendaring
Traditionally somewhat related to mail, although mostly through orgmode these days, I want a simple setup to pull in some calendaring information. I run a radical calendar server somewhere, but having the same info in orgmode makes sense for me.
...
Keyboard binding are the primary way to interact for me. I have been struggling with consistent keyboard shortcuts and how to integrate them with the other systems on my machine which capture shortcut keys. At this time the following applications capture shortcut keys:
...
#+begin_src emacs-lisp
  ...
  (use-package mu4e
    ...
    ;; The empty field is to accommodate for mu4e thread folding symbols
...
#+end_src
** IRC
I run a weechat proxy on a vps for IRC. Luckily there is a good emacs package to use irc in emacs directly.

#+begin_src emacs-lisp
  (use-package weechat
    :if (version< "28" emacs-version)     ;patch for 28 does not work for earlier versions

    ;; this works, but we /are/ inside weechat when we need this, can't it be auto-loaded?
