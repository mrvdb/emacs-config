...
#+BEGIN_SRC emacs-lisp
  ...
  ;; Dynamic setting for tags normally, align with wrapping column
  ;; Interestingly `window-body-width` works for this
  ;; TODO: this is not enough, because we dont always change focus, why not have it on an idle timer or after some event?
...
#+BEGIN_SRC emacs-lisp
  ...
  (defun mrb/message-mode-hook ()
    ...
    (setq visual-fill-column-width nil) 	; So `fill-columns gets used
