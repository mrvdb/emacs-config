...
Last update: <2020-05-21 18:49:05>
...
#+begin_src sh :exports code :tangle ~/bin/edit :shebang #!/bin/bash
  ...
  # Normal emacsclient as $EDITOR is fine except:
  # 1. no filename is given -> just open a frame
  # 2. stdin is given '-'   -> don't edit the file '-' but treat it as stdin

  # Default behaviou is:
  # - if not otherwise specified, reuse exisiting frames
  # - we're not waiting

  # Syntax: emacsclient [OPTIONS] FILE...
  # FILE can be FILENAME or [+LINE[:COLUMN]] FILENAME
  EC=`which emacsclient`

  # Default arguments
  ARGS="--no-wait --quiet --suppress-output"

  # 1. New frame if we're just called without arguments
  [ $# -eq 0 ] && ARGS=$ARGS' --create-frame'

  # 2. Handle special 'stdin' case
  if [ $# -ge 1 ] && [ "$1" == "-" ]; then
