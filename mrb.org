...
recipes in an easy way.

I have now converted package management and delegated it to Cask. From
this configuration point of view, everything needed should be there.
The main cask configuration is in =init.el=

The reasons for converting my el-get based setup to cask:
1. separate the pure package management to a tool specifically
   designed for that
2. be able to use use-package later on.
3. configuration seemed cleaner.
...
#+BEGIN_SRC emacs-lisp
  ...
  (mrb/fix-fontset-font "Symbola" #x1F4A9) 
  ;; PILE OF POO symbol 💩 \larr This should now show a pile of poo if all is well
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  (bind-key [(super .)] 'org-todo org-mode-map)
  ...
  (bind-key [(control super .)] 'mrb/force-org-todo org-mode-map)
  (bind-key [(control super .)] 'mrb/force-org-agenda-todo org-agenda-mode-map)
  (bind-key [(super .)] 'org-agenda-todo org-agenda-mode-map)
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  ...
  (bind-key [(super t)] 'org-schedule org-mode-map)
  (bind-key [(super t)] 'org-agenda-schedule org-agenda-mode-map)
  (bind-key [(meta p)] 'org-set-property org-mode-map)
  (bind-key [(meta p)] 'org-set-property org-agenda-mode-map)
  (bind-key [(control super s)] 'org-save-all-org-buffers org-mode-map)
  ...
  (bind-key "\C-cs" 'mrb/shortenurl-replace-at-point org-mode-map)
...
#+BEGIN_SRC emacs-lisp
  ...
  (bind-key [(control +)] 'text-scale-increase)
  (bind-key [(control -)] 'text-scale-decrease)

  ;; Line handling functions
  (bind-key [(super \`)] 'toggle-truncate-lines)
  ...
  (bind-key [(super /)] 'comment-or-uncomment-region)

  (bind-key [(super s)] 'save-buffer)

  ;; Kill buffer, FIXME: asks for name, which I dont need
  (bind-key [(super k)] 'ido-kill-buffer)
  ...
  (bind-key [(super z)] 'undo)
  (bind-key [(super x)] 'clipboard-kill-region)
  (bind-key [(super c)] 'clipboard-kill-ring-save)
  (bind-key [(super v)] 'yank)
  (bind-key [(super a)] 'mark-whole-buffer)
  ...
  (bind-key [(super n)] 'mrb/new-empty-buffer)
...
#+BEGIN_SRC emacs-lisp
  (bind-key [(control x) (control q)] 'save-buffers-kill-emacs)
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  (bind-key [(control c)(meta x)] 'execute-extended-command)
  (bind-key [(meta x)] 'smex)
  (bind-key [(meta X)] 'smex-major-mode-commands)
#+END_SRC

I want to be able to call up magit from anywhere

#+BEGIN_SRC emacs-lisp
  (bind-key [(control c) (m)] 'magit-status)
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  ...
  (bind-key [(meta super left)] 'previous-buffer)
  (bind-key [(meta super right)] 'next-buffer)

  ;; Ctrl Cmd moves buffers up and down in the tiling of emacs
  (bind-key [(control super up)]   'buf-move-up)
  (bind-key [(control super down)] 'buf-move-down)
  ...
  (bind-key [(super up)] 'move-border-up)
  (bind-key [(super down)] 'move-border-down)
#+END_SRC
*** Org-mode
Orgmode specific keybindings.

#+BEGIN_SRC emacs-lisp
  (bind-key [(control c) (g)] 'mrb/gtd)
  (bind-key [(control c) (a)] 'org-agenda)
  (bind-key [(control c) (b)] 'org-iswitchb)
  (bind-key [(control c) (l)] 'org-store-link)
  (bind-key [(control c) (j)] 'mrb/make-journal-entry)
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  (bind-key [(control c) return] 'eshell)
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  ...
  (bind-key [(control c) (ip)] 'identica-update-status-interactive)
  (bind-key [(control c) (id)] 'identica-direct-message-interactive)
  (bind-key [(control c) (is)] 'identica-shortenurl-replace-at-point)
  (bind-key [(control c) (d)]  'mrb/make-dent-frame)
...
#+BEGIN_SRC emacs-lisp
  (use-package elfeed
    ...
    :config
    (progn
      (defun mrb/elfeed-watchlater()
        (interactive)
        (elfeed-search-tag-all 'watchlater))
      ;; Bind 'w' to add watch later tag (TODO: use a toggle?)
      (bind-key "w" 'mrb/elfeed-watchlater elfeed-show-mode-map)
      (bind-key "w" 'mrb/elfeed-watchlater elfeed-search-mode-map)
...
#+BEGIN_SRC emacs-lisp
  ...
  (defvar elfeed-feeds-alist
        '(("http://gdata.youtube.com/feeds/base/users/mvanderboom/newsubscriptionvideos")
          ("bridgeport" marktplaats)
          ("abene" marktplaats)
          ("plasmasnijder" marktplaats)
          ("cazeneuve" marktplaats)
          ("deckel" marktplaats)
          ("lista" marktplaats)
          ...
          ("http://jaskaleimi.blogspot.com/feeds/posts/default")
          ...
          ("http://www.veiligheidsblog.nl/feed/")
...
[2014-05-20 di] worked with it for a while and it's pretty
