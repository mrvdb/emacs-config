...
Last update: <2020-06-03 15:46:40>
...
#+begin_src emacs-lisp
    ...
    (defun mrb/mu4e-action-view-in-eww (msg)
      "View the body of MSG in internal web"
      (eww-open-file (mu4e~write-body-to-html msg)))

    (add-to-list 'mu4e-view-actions
                  '("E - view in eww" . mrb/mu4e-action-view-in-eww) t)
...
For displaying diffs in a bit more fancy way, I normally use [[https://github.com/dandavison/delta][delta]]. The =magit-delta= package is a wrapper around it. The main advantage of delta over diff is language awareness, which allows for syntax highlighting in diffs.
  ...
  (use-package magit
    ...
    :init
      (use-package magit-delta
        :config
        ;; FIXME: why does magit-delta not by default take the delta theme?
        (setq magit-delta-default-dark-theme "MrB")
