...
Last update: <2022-01-07 12:56:27>
...
1. =early-init.el= emacs' early initialisation file;
...
#+begin_src sh :exports code :tangle ~/bin/edit :shebang #!/bin/bash
  ...
  if ( ! systemctl -q --user is-active emacs );then
      notify-send "Emacs server not found, starting...."
  ...
  if [ $DISPLAY ]; then
      ...
      [ `expr match "$ME" '.*-frame'` -ne 0 ] && ARGS=$ARGS' --create-frame'
...
As I want to have version controlled package installation that is easily reproduceable, =straight.el= seem a logical choice, so let's start by bootstrapping that.
...
#+begin_src emacs-lisp
  ...
  (setq use-package-always-ensure nil      ; Make sure this is nil, so we dont use package.el
        use-package-always-demand t        ; FIXME: only when using deamon?
        )
...
#+begin_src emacs-lisp
  (use-package use-package-ensure-system-package)   ; Need this because we are in use-package config
...
#+begin_src emacs-lisp
  ...
  (use-package password-store)            ; Missing auto dependency?
...
#+begin_src emacs-lisp
  (use-package pdf-tools
    ...
    :bind (:map pdf-view-mode-map
                ...
                ("<S-return>" .  'newline))
    :config
    (pdf-tools-install) ; :no-query auto builds epfinfo when needed
    (require 'pdf-annot)
...
In quite a few situation I would like to use keybinding which are only valid when a selection/region is active.
...
#+begin_src emacs-lisp
  (use-package selected
    ...
    :after (org)
    :init
    (setq selected-org-mode-map (make-sparse-keymap)) ;create map for selected regions
    (setq selected-org-journal-mode-map (make-sparse-keymap))
    :commands selected-minor-mode         ; do I want the global one too?
    :bind ( :map selected-keymap           ; bindings for all active regions
            ("q" . selected-off)
            ("u" . upcase-region)
            ("d" . downcase-region)
            :map selected-org-mode-map
            ;; Used by the selected package FIXME I want this configured with org, not here
            ("~"       . (lambda () (interactive) (org-emphasize ?~)))
            ("_"       . (lambda () (interactive) (org-emphasize ?_)))
            ("*"       . (lambda () (interactive) (org-emphasize ?*)))
            ("+"       . (lambda () (interactive) (org-emphasize ?+)))
            :map selected-org-journal-mode-map
            ;; Used by the selected package FIXME I want this configured with org, not here
            ("~"       . (lambda () (interactive) (org-emphasize ?~)))
            ("_"       . (lambda () (interactive) (org-emphasize ?_)))
            ("*"       . (lambda () (interactive) (org-emphasize ?*)))
...
From now on we can bind keys into the selected-keymap if we want keys to be active on every regions. When only needed for a certain major mode, define a keymap named =selected-<major-mode>-keymap= and bind keys into that.
...
For even more esoteric characters we have to do some extra work. No font provides all Unicode characters. While I previously had a manual solution, I'm now relying on the package =unicode-fonts= to do the proper mapping. The first run will be very slow, but the information gets cached.
...
Miscelaneous other visual settings follow.
...
Autosave helps to automatically recover to the latest save point, but I need a bit more. This is done through versioned backup file. The recovery process is then manual though.

#+begin_src emacs-lisp
  (setq
   create-lockfiles nil                   ; just clutters for me, no real use?
...
The autosave helps for the minor disasters, my backups help for the major disasters. What else is needed is a 'normal save' but automatically when I forget to do this.
...
#+begin_src emacs-lisp
  (use-package dired
    :straight (:type built-in)                                 ;builtin
    :init
...
#+begin_src emacs-lisp
  (use-package org
    :straight (:includes (org-capture org-crypt org-datetree org-protocol))
    :mode ("\\.txt\\'" "\\.org\'")
...
My policy is to use Emacs for as many things and use as little other programs as necessary. All this within reason obvously.
...
There's a special place for the =nyxt= browser here. It is particularly interesting because its design was inpirated by emacs. The two thing that are interesting to me are that it is programmed in common-lisp and thus has alle the niceties of that, including an option to connect a REPL to the browser and change the program while it is running. So, let's start with a convenience function to make a connection to a runing Nyxt instance.
...
#+begin_src emacs-lisp
  ...
  (use-package mu4e
    :straight (:type git :host github :repo "djcb/mu"
                     ...
                     :includes (org-mu4e mu4e-icalendar))
    ;; Make sure we use what was just compiled
    ...
    :init
    (fullframe mu4e mrb/mu4e-quit)                  ; Claim the whole screen

    :config
    ; Use local thread-folding package
    ; This used to give some problems with other packages?
    (use-package mu4e-thread-folding
      :straight (:host github :repo "rougier/mu4e-thread-folding")
      :init
      ;; Make sure we have our custom empty field to put the symbols in place
      ...
      :config
      ;; This needs to be here, it's too early in the visual section
      (set-face-attribute 'mu4e-thread-folding-root-unfolded-face nil :background "#434c5e")
      (set-face-attribute 'mu4e-thread-folding-child-face nil :background "#4c566a"))
    ...
    ;; Try to get some more icons in mu4e to spice up the display
    (use-package mu4e-marker-icons
      :init
      ;; This is needed for me,otherwise the lineheight is fucked up
      (setq mu4e-marker-icons-use-unicode 0)
    ...
    (setq mu4e-bookmarks
          (list
           ...
           (make-mu4e-bookmark :name "Todays messages"
                               ...
                               :key ?d)
           (make-mu4e-bookmark :name "Trash"
                               :query "maildir:/Trash"
                               :key ?t)
           (make-mu4e-bookmark :name "Junk"
                               :query "maildir:/Junk"
                               :key ?j)))
    ...
    ;; The empty field is to accomodate for mu4e thread folding symbols
...
#+begin_src emacs-lisp
  (use-package weechat
    :if (version< "28" emacs-version)     ;patch for 28 does not work for earlier versions

    ;; this works, but we /are/ inside weechat when we need this, can't it be autoloaded?
    :straight (:includes (weechat-tracking weechat-notifications weechat-image)
               :fork t)
...
The location of the eldoc information in the message area is a bit far away from the editting point. I have looked at eldoc-overlay and others to resolve that, but haven't found a satisfying solution yet.
...
#+begin_src emacs-lisp
  (use-package go-mode
    ;; FIXME: how to do this with :straight
...
#+begin_src emacs-lisp
      (use-package geiser
        ;; repository moved and straight.el does not know about it yet
        ...
        (use-package geiser-chez
               :straight (:host gitlab :repo "emacs-geiser/chez")
               :config
...
#+begin_src emacs-lisp
  (use-package sly
    ...
    (add-hook 'sly-mrepl-mode-hook 'override-sly-repl-bindings-with-paredit)
    ...
    ;; Set up fontlocking
    (add-hook 'sly-mrepl-mode-hook 'sly-mrepl-font-lock-setup)
...
**** Github gists
I use github gists sometimes
#+begin_src emacs-lisp
  (use-package gist)
#+end_src

**** Grepping files
Searching for terms in files is a hard problem, but the vast majority of my usecases can just be solved by searching for terms in the current project root, almost always defined by having a .git directory in a parent directory. So, let's start there and see what else I'll need.
...
#+begin_src emacs-lisp
  (use-package helm-ag
    ...
    (defun mrb/search ()
      (interactive)
      ;; but do *not* use the vcs ignore mechanisme
