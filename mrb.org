...
Last update: <2020-05-22 08:19:53>
...
#+begin_src sh :exports code :tangle ~/bin/edit :shebang #!/bin/bash
  ...
  if [ $DISPLAY ]; then
      ...
      ARGS=$ARGS' --create-frame --no-wait'
  else
...
I'm now starting to explore running and (auto-) re-starting emacs as a systemd service. This is fairly easy, but the details can be finicky. For example, I have to override the systemd unit file with a =SSH_AUTH_SOCK= environment variable to allow magit to push (which uses ssh).
...
1. I want to have at least one server process for 'normal' editting so emacs does not have to start a process for every file; the goal here is to have fast opening of files to edit. This is critical. I'm assuming that for this a server-like-setup is needed.
...
*** So, given the above, how to proceed?

[[https://www.gnu.org/software/emacs/manual/html_node/emacs/Emacs-Server.html#Emacs-Server][According to the manual]] there are several ways to run the server:

1. run the emacs-lisp command =(server-start)= in the initialisation of emacs or interactively after starting;
2. use the =--daemon= command line arguments to run emacs in daemon mode;
3. use systemd to start it (like 2. but under systemd's control instead of manually)
4. create a listening socket, which starts up emacs once a client tries to connect to it.


I think the first method is out, although I like having everything under control inside Emacs, because when the last frame exits, the server will exit and that's probably not what I want.

Assuming thusly I want the daemon to be started without having a frame that can be exited, there's a need for other criteria to make the decision:

- if opening a file, should one frame be used?, always?
- should there be 'one entry point' like 'edit' and have every program and person use that to get to emacs, or is it ok to specialize the tools. (e.g. auto matic edit runs by git/magit/systemd edit etc.
-

Actions:
- ⌘e :: get an editor window right away, either a new frame, or a terminal editor
- edit <somefile> :: open =<somefile>= I'm leaning towards opening a new frame, always.

*** Affected configs:

- ~/.emacs.d/mrb.org
- ~/.config/zsh/.zshrc has aliases and environment variables
- ~/.xbindkeysrc.scm has main shortcut key
-
