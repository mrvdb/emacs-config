...
* Emacs configuration                                              [2/96]                      :emacs:exclude:
:PROPERTIES:
:CATEGORY: Emacs
:END:
...
:END:
** TODO find a working fill-column indicator which does not kill performance
:PROPERTIES:
...
I've settled on gnu global as this has excellent emacs support, but is
largely independent from it.
...
** TODO Massage line continuation/truncation for jabber-chat mode                                       :fix:
:PROPERTIES:
:CREATED:  [2015-05-19 di 11:32]
:END:
- wrapping and not having the continuation chars makes sense in this
  mode
- preferably wrap with indent to the width of the users nickname
...
** TODO Mail compose window has side-effects of notmuch                                                 :fix:
:PROPERTIES:
:CREATED:  [2015-05-07 do 12:02]
:END:
After notmuch started, client is different?

Probably going to use the wanderlust composer
** TODO Look at fold-this.el package                                                                  :check:
:PROPERTIES:
:CREATED:  [2015-04-29 wo 20:43]
:END:
Can fold/unfold arbitrary region. The difficulty then becomes to
create those regions effectively, context dependent.
...
:END:
** TODO Instead of using edit-wait, create a script for capture-mail, which uses its own frame        :build:
:PROPERTIES:
:CREATED:  [2015-06-04 Thu 11:39]
:END:
