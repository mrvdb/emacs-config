...
Last update: <2022-02-12 16:16:13>
...
I have used =org-caldav= but even in just a read only setup it does not work properly. I'm using =ical2orgpy= which is a pythons script that takes =.ics= input and creates an org-mode file.
...
#+begin_src sh :exports code :tangle ~/bin/ical2org :shebang #!/bin/sh
    ...
    WGETBASE="/usr/bin/wget -O wget.log -O - --quiet"

    # Just go over each one manually
    # FIXME make this nice with url . file pairs in a loop
    cd $BASEDIR

    $WGETBASE https://calendars.hsdev.com/mrb/e00ffed0-2fc3-4adb-ba08-95c93b2b6abc/ 2>/dev/null | $ICAL2ORGPY - marcel.org
    $WGETBASE https://calendars.hsdev.com/mrb/shared-sylvia/ 2>/dev/null | $ICAL2ORGPY - sylvia.org
    $WGETBASE https://calendars.hsdev.com/mrb/4de0fb18-8548-61ce-57fc-b2319b87106d/ 2>/dev/null | $ICAL2ORGPY - afval.org
#+end_src
