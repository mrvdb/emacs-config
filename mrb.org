...
#+BEGIN_SRC emacs-lisp
  ...
  (use-package guide-key )     ;; when pressing a prefix key, show the rest after a timeout

  ;; Show color of #RRBBGG texts
  (use-package rainbow-mode
    :diminish)

  (use-package edit-server ) ;; Do I still need this? Is is the counterpart to a plugin in browser?

  ;;; Not sure if this is still needed if i configure help/ido/etc.
...
(use-package pdf-tools
  ...
  :config
  ;; All thi   s came from http://pragmaticemacs.com/emacs/even-more-pdf-tools-tweaks/
    ...
    (use-package base16-theme
      ...
      (setq base16-adjustments
            '(
              ...
              (region                       :foreground nil :background base02 :inherit nil :weight normal)
              ;; Magit
...
#+BEGIN_SRC emacs-lisp
  (use-package highlight-parentheses
    ...
    (setq hl-paren-colors
          ;; FIXME: reformulate this in terms of face-attributes of outline faces
  ...
  (use-package spaceline-config
    ...
    :config
    ;; I want the cursor the same as the highlight face of the modeline
    (setq mrb-cursor-color
          (face-attribute 'spaceline-highlight-face :background))
...
I'm not planning on leaving emacs because it's strengths and my long
...
#+BEGIN_SRC emacs-lisp
  ...
  ;; Add notion of doing an acount with the stuff surrounding a text object
...
#+BEGIN_SRC emacs-lisp
  ...
  (defun mrb/make-mailforward-frame (mids)
    "Create a frame in which we forward all messages in the list
    `mids`. The argument is a list of strings having the format:
     (id:<message id>)"
    (interactive)
    (mrb/make-mailframe)
    (notmuch-mua-new-forward-messages (list mids))
    (delete-other-windows))
#+END_SRC
