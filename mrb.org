  ...
  (use-package base16-theme
    ...
    (setq base16-adjustments
          '(
            ...
            (elfeed-search-date-face      :foreground "#5e81ac")
...
#+BEGIN_SRC emacs-lisp
  ...
  ; Autofill should be off for a couple of modes, but mostly on.  I'm
  ; backpedaling a bit on this, visual-line mode is a lot smarter to use
  ; in many cases. I'm mainly wanting the fill mode for visual reasons,
  ; not actual line breaking
  (auto-fill-mode 1)
...
A snippet which resized the agenda to its window whenever its rebuilt
by orgmode
...
I'm using helm which has a replacement for the =M-x= command, see the
helm config for details.
...
For most of the completion that is needed which is not inline (for
which I am using the =company= package, helm seems to be the most
powerful solution. I used to use ido, but helm seems a lot easier and
more broad.
...
#+BEGIN_SRC emacs-lisp
 ...
 (use-package smtpmail
    ...
    (setq
     ...
     ;; User agent style is message mode
...
#+BEGIN_SRC emacs-lisp
  (defun mrb/message-mode-hook ()
    ...
    ;; Use visual line mode and visually break at 66 which
    ...
    ;; Make sure we show the line wrappers in the right fringe area
    (setq visual-line-fringe-indicators '(nil right-curly-arrow))

    ;; Adapt fill column to what encoding would do (not actually needed)
    ;; and make visual-mode adhere to it.
    ...
    ;; For displaying of f=f messages I'm not sure what is needed.
    ;; This would be `notmuch-show-mode` ?
  ...
  (use-package notmuch
    ...
    (defun mrb/nm-refile()
      (interactive)
      ;; TODO: make this elisp only
      ;; This uses rofi to select folders
      ;; Shell out for now to the refile script
...
#+END_SRC
** Mastodon
Not messaging or chatting perse, but a microblog social network.

#+begin_src emacs-lisp
;; Mastodon in emacs
(use-package mastodon
  :config
  (setq mastodon-instance-url "https://todon.nl"
...
I think most of my documentation needs can be fulfilled with
=(helm-)dash=
...
While emacs itself has a very nicely documentation system, I still
don't understand why this is not interpolated into every other mode
and or language.

Example: =C-h-f= in lisp-mode describes the function to me. If I am in a
python file, I want it to do exactly the same, but for the python
function.
There's a page on emacswiki dealing with this a bit:
https://www.emacswiki.org/emacs/Context_sensitive_help

*** RFC document reader
I often need to consult RFC documents, so not having to leave emacs
for that and be able to use all tools on RFC documents is very
helpful.
...
** Gcode
I'm not in the habit of editting gcode, but every now and then I need
to look at it and I want it reasonably decent. My usecase is just
sending gcode to my 3D-printer which runs Marlin firmware.

Using =define-generic-mode= is more than enough to define something
useful for me.
...
#+end_src
** Openscad
Openscad is a script based 3d parametric drawing program. Obviously
I'm editting those scripts in emacs.
...
#+end_src
** SQL
SQL has a number of different dialects I use, depending on the
...
git as the revision control system. In emacs that translates to using
magit, so let's begin with bringing that in.

Magit forge is a submodule for magit which helps in managing
repositories in forges (notably github and gitlab). This looks very
promising, albeit slow.
