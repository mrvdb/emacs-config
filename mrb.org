...
Last update: <2022-01-18 12:03:24>
...
#+begin_src emacs-lisp
  ...
  (setq use-package-always-ensure nil      ; Make sure this is nil, so we do not use package.el
        use-package-always-demand t      ; Until I grok the loading order logic I need this
...
#+begin_src emacs-lisp
  (use-package selected
    ...
    :demand t                             ; Does not load without demand, why not?
    :init
    (setq selected-org-mode-map (make-sparse-keymap)
          selected-org-journal-mode-map (make-sparse-keymap))
    :commands (selected-global-mode selected-minor-mode)
...
#+begin_src emacs-lisp
  ...
  (setq org-agenda-text-search-extra-files
     `('agenda-archives ,(directory-files
...
#+begin_src emacs-lisp
  (defun mrb/capture-todo ()
    "Capture a TODO item"
    (interactive)
    ;; FIXME: use t by default, but with C-u, use l ?
    ;; Problem is that this is called from the environment, not from within Emacs
    (org-capture nil "t"))
...
The elements of org babel are blocks of source code. Here are some helper programs relevant for those blocks:
