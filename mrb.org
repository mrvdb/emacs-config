...
* Emacs configuration                                              [1/84]                      :emacs:exclude:
:PROPERTIES:
:CATEGORY: Emacs
:END:
...
** TODO Construct a bootable usb system with emacs on it, rsynced  with my config on the main machine :build:
:PROPERTIES:
:CREATED:  [2012-10-10 wo 19:49]
:END:
Idea is to take this with me and be able to test out keyboards
whenever I want.

Ideally, having a usb powered pocketsize computer would be even
better.
