    ...
    (setq mrb/packages
          (append
           ...
           '(smex apache-mode gnuplot-mode sudo-save
                  ...
                  git-timemachine eimp litable diminish paredit fullframe wanderlust avy adaptive-wrap)
...
- break at the word boundary instead of the character boundary

#+BEGIN_SRC emacs-lisp
  (global-visual-line-mode)                ;; Turn on visual line mode by default
  (setq-default truncate-lines t)          ;; Don't truncate lines
  (setq truncate-partial-width-windows nil);; also not in partial width windows
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  (defun mrb/jabber-chat-mode-hook ()
    ...
    ;; First, set the adaptive wrap extra inden to skip the '[HH:MM] ' string, i.e. 9 places
