...
#+BEGIN_SRC emacs-lisp
  ;;; There is a sudo-edit package which seems more maintained, shoud I use that?
...
#+BEGIN_SRC emacs-lisp
  ;;; Make the default input the standard TeX method
  ...
  (use-package base16-theme
    ...
    (setq base16-adjustments
          '(
            ...
            (show-paren-match             :foreground nil :background base0F :inherit nil :weight normal)
...
#+BEGIN_SRC emacs-lisp
    ...
    (use-package spaceline-config
      ...
      :config
      ;; Make the highlight in the modeline dependent on the evil state
      (setq spaceline-highlight-face-func 'spaceline-highlight-face-evil-state)

      ;; And I want the cursor to follow that, but I don't know how to do that
      ;; automatically, so here's the mapping.
      ;; FIXME: rewrite to automatic
      (setq evil-normal-state-cursor  '("#bf616a" box)
            evil-insert-state-cursor  '("#ebcb8b" box)
            evil-emacs-state-cursor   '("#a3be8c" box)
            evil-motion-state-cursor  '("#81a1c1" box)
            evil-visual-state-cursor  '("#8fbcbb" box)
            evil-replace-state-cursor '("#b48ead" box))
    ...
    (use-package paren
      ...
      (show-paren-mode 1)
      ;; Show paren if it is visible, region otherwise
...
#+BEGIN_SRC emacs-lisp
  (use-package org-capture
    ...
    :ensure nil
    :config
    ; Need to use hook here, org-capture-mode is not a major mode
...
#+BEGIN_SRC emacs-lisp
    (setq
     ...
     (quote (
             ...
             ("z" "Ziggo logbook entry"
              item (id "ziggo-logbook") " %U %?\n%i\n"))))
#+END_SRC
...
For most of the calendaring I use google calendering which integrates
nicely with org-mode through the =org-gcal= package.
A timer takes care of fetching new entries. For now, the sync
is only one way (google \rarr org-mode) and not the other way around.

#+BEGIN_SRC emacs-lisp
  (defun orgcal (file)
    (concat "~/dat/org/_calendars/" file))

  (defun mrb/gcal-fetch()
    (interactive)
  ...
  (use-package org-gcal
    ...
    (setq
     ...
     ;; 30 days in the past, 90 days into the future
     org-gcal-up-days 30
     org-gcal-down-days 90
     org-gcal-client-id "897495986604-hchkmahh315dle855haoabe2sh11bvds.apps.googleusercontent.com"
     org-gcal-client-secret "a39Yw1Y-9hapwPZAbbY6Fdsm"
     org-gcal-file-alist `(("mvanderboom@gmail.com"  . ,(orgcal "marcel.org"))
                           ("qbb7fokc5qjl8dbfr5ol5sikqc@group.calendar.google.com"  . ,(orgcal "afval.org"))
                           ("hpjq29s4ma912rr504n1vj0of8@group.calendar.google.com" . ,(orgcal "planner.org"))
                           ("sylviadeswart@gmail.com" . ,(orgcal "sylvia.org"))))

    (run-with-idle-timer 120 'always #'mrb/gcal-fetch))
...
#+BEGIN_SRC emacs-lisp
  (use-package org-journal
    ...
    (setq
     ...
     org-journal-carryover-items "+CARRYOVER=\"yes\"|+TODO=\"TODO\""
...
#+BEGIN_SRC emacs-lisp
  ...
  (use-package which-key
    ...
    (setq which-key-idle-delay 2.0
          ;; Experimental: also complete for evil operators like d,y,g etc.)
...
emacs, is the ultimate navigational package.

*** Evil mode
Evil mode is a Emulated VI Layer on top of emacs. Arguably the
strenght of vi is in its composeability of text editting commands. I
want to try this out as a component in Emacs.
I'm not planning on leaving emacs because its strengths and my long
history with it is not something I want to lose, but it seems to me
that having evil-mode as the base navigational logic can make a number
of packages obsolete (=avy-jump=, =expand region=, possbibly
=multiple-cursors=)
...
#+BEGIN_SRC emacs-lisp
  ...
  (use-package evil
    ...
    :init
    (setq evil-want-integration nil) ; Need this for evil-collection, no idea what it does.
    :config
    (setq evil-move-cursor-back nil) ; When exiting insert mode do not move cursor!!

    ;; I'm still chickening out apparently, but I still need this.
    (setq evil-default-state 'emacs)
    (evil-mode 1))

  ;; Map as much as possible for all packages when in Normal mode
  ;; We still can C-z to emacs state when in need
  (use-package evil-collection
    :after evil
    :config
    ;; Exclude list of modes
    (delete 'notmuch evil-collection-mode-list)
    (delete 'elfeed evil-collection-mode-list)
    (evil-collection-init))

  ;; Add notion of doing an action with the stuff surrounding a text object
  (use-package evil-surround
    :after evil
    :config
    (global-evil-surround-mode 1))

  ;; Add notion of commenting something as action
  ...
  (use-package notmuch
    ...
    (setq
       ...
       notmuch-saved-searches '((:name "unread" :query "tag:unread" :key "u")
                                ...
                                (:name "INBOX" :query "query:INBOX" :key "i")
                                (:name "TODO" :query "query:TODO" :key "$"))
    ...
    (add-to-list 'evil-emacs-state-modes 'notmuch-search-mode)
...
** Twitter
There seems to be a defacto standard for twitter in emacs, which is
=twittering-mode=. I have made a simple configuration for it, all of
which is below, except for the status format, which is saved in
custom.el for now until it stabilizes.
