...
For file management itself, dired is the defacto standard, but I do not use it much really. To help me out with keys and commands, I'm adding the =casual-dired= package.
...
Not really man pages as such, but meant as their /advanced/ cousin for GNU programs is the info format. As I tend to forget all the key shortcuts, install =casual-info= as well to help with that. =C-o= seems to be the defacto standord to summon the transient menu in a mode.
