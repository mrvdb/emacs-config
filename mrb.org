...
#+BEGIN_SRC emacs-lisp
  ...
  (defun run-client-settings()
    ...
    ;; TODO: make all these in a list and loop over them
    ;; When making a selection, keep all font-locking too, but make it
    ;; stand out from the matching background, the key here is the nil in the
    ;; foreground specification
    (set-face-attribute 'region nil :inherit nil :background "gray32" :foreground nil)
  
    ;; Just change the background when matching the expression, leave
    ;; other fontifying as is
    (set-face-attribute 'show-paren-match nil :inherit nil :background "gray32" :foreground nil)

    ;; Similar for magit-highlight
    (set-face-attribute 'magit-item-highlight nil :inherit nil :background "gray32" :foreground nil)
