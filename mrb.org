...
Last update: <2020-05-22 10:49:37>
...
#+begin_src sh :exports code :tangle ~/bin/edit :shebang #!/bin/bash
  ...
  if [ $DISPLAY ]; then
      ...
      # Add no-wait, unless we're specifically called with '-wait'
      [ ! ${ME: -5} == "-wait" ] && ARGS=$ARGS' --no-wait'
  else
...
#+begin_src sh :exports code :tangle ~/bin/mailto-handler :shebang #!/bin/bash
  ...
  emacsclient --socket-name=default -n --eval "$elisp_expr"
#+end_src
...
emacsclient --socket-name=default -n -e '(mrb/compose-mail)'
#+end_src
