...
(note: this document was posted automatically with the [[https://github.com/dangom/writefreely.el][writefreely.el]] Emacs package. Therefore, some layout issues may occur due to orgmode to markdown conversion.
  ...
  (use-package base16-theme
    ...
    (setq base16-adjustments
          '(
            ...
            (show-paren-match             :foreground "#eceff4" :background "#5e81ac" :inherit nil :weight normal)
            (show-paren-mismatch          :foreground "#2e3440" :background "#bf616a" :inherit nil :weight normal)
            ;; Set regions so we keep the normal font locking
            (region                       :foreground nil :background "#3b4252" :inherit nil :weight normal)
            ;; Magit
            ...
            (flyspell-duplicate           :inherit flycheck-warning :foreground "#2e3440")
            ...
            ;; Notmuch faces
            (notmuch-message-summary-face :foreground "#3b4252" :background "#88c0d0")

            ;; Parent of all tag faces we customize
            (notmuch-tag-face  :foreground "#a3be8c" :height 100)
            (notmuch-tree-match-tag-face :inherit notmuch-tag-face)
            (notmuch-tree-no-match-tag-face :inherit notmuch-tag-face)
            (notmuch-tag-added :inherit notmuch-tag-face :underline "green")
            (notmuch-tag-deleted :inherit notmuch-tag-face :underline "red")
            (notmuch-tag-flagged :foreground "#ebcb8b" :height 100)

            (notmuch-message-summary-face :background "#5e81ac" :foreground "#d8dee9")
            (notmuch-search-flagged-face :foreground "#88c0d0")
            (notmuch-tag-unread :inherit nil :foreground "#ebcb8b" :weight bold)
            (notmuch-tree-match-author-face :foreground "#88c0d0")
            (notmuch-tree-match-tag-face :inherit notmuch-tag-face)

            (notmuch-tree-no-match-face :foreground "#4c566a")
            (notmuch-tree-match-face :foreground "#d8dee9")
            ...
            ;; Make highlights actually highlights, not hard to find.
            (hl-line :background "#5e81ac")))
...
#+BEGIN_SRC emacs-lisp
  ...
  to find a file and location where a caputured ITEM should
  be stored."
...
A snippet which resized the agenda to its window whenever its rebuilt by orgmode

#+begin_src emacs-lisp
;; Goal: have the agenda window sized properly
...
** Journaling
While the capture method is useful I found myself not using it very much. Not sure why, but I'm going to try =org-journal= and see it that is a better fit.
...
#+BEGIN_SRC emacs-lisp
  (use-package org-journal
    ...
    (setq
     org-journal-dir (expand-file-name "/media/data/org/journal/")
...
Once that is in place files can be opened with =/ssh:/host:/path/to/file= syntax. Surprisingly, the syntax I got used to in the past (i.e. =/host:/path/to/file=) does not work for my anymore.
...
#+BEGIN_SRC emacs-lisp
  ...
  ;; CHECK: how do we deal with this using mu4e? Needed?
...
#+BEGIN_SRC emacs-lisp
  (use-package message
    ...
    (setq
     ...
     ;; What I would like:
     ;; - Add '> ' when quoting message lines
     ;; - Add '>' when quoting cited lines (uses `message-cite-prefix-regexp` to determine this)
     ;; - nothing on empty lines
     message-yank-prefix "> "
    ...
    (defun mrb/message-mode-hook ()
      ...
      ;; Auto-fill makes no sense anymore now, and will in fact mess
      ;; everything up, so make sure it is off.
      (auto-fill-mode 0)

      ;; Adapt visual fill column locally to a reasonable value
      ...
      ;; Add some color to quoted lines
...
The cop-out intermediate solution is address completion by notmuch itself, which has proven to work fine so far.
...
#+BEGIN_SRC emacs-lisp
  ...
  (use-package notmuch
    ...
    (setq
     ...
     notmuch-crypto-process-mime nil ;; GPG tends to block a long time, so I want it manual
     ...
     ;; This looks counterintuitive but it works like this:
     ;; When there is a text/html part and a text/plain part,
     ;; in almost all of the cases it's better to have me
     ;; render the text/html part to text in emacs than to show
     ;; the text/plain part directly. It will usually be full of and
     ;; with shr rendering the html I get at least a neat representation
     ;; I saw this break down for me when the two parts are different, but I could
     ;; argue that the sender is at fault there.
     mm-text-html-renderer 'shr  ;; this makes sure we have reasonable rendering of html
     ;;  Now favour the html part over text
     notmuch-multipart/alternative-discouraged '("text/plain" "text/html")
     notmuch-message-forwarded-tags '("+passed")

     ;; Customize tag configuration, visual
     notmuch-tag-formats
      '(("unread"
         (propertize tag
                     (quote face)
    ...
    ;; In any case, remove new since we're handling it.
    ;; Note that this disables the possibility to toggle the 'new' tag.
    (defun nm-tag-toggle (tag currenttags)
      (list "-new" (concat (if (member tag currenttags)
                               "-" "+") tag)))
    ...
    ;; When notmuch starts, give it the full frame
    (fullframe notmuch notmuch-bury-or-kill-this-buffer)

    ;; Refile thread or message
    ;; Depends on ~/bin/mailrefile
    (defun mrb/nm-refile()
      (interactive)
      ;; TODO: make this elisp only, This uses rofi to select folders
      ;; TODO: make this work with >1 selected
      (let ((process (start-process
        ...
        (set-process-sentinel process 'on-finish-mailrefile)))

    (defun on-finish-mailrefile (process e)
      "Refresh the notmuch buffer when refiling is done"
...
#+BEGIN_SRC emacs-lisp
  (defun mrb/notmuch-address-selection-function (prompt addressess first)
    "Use `completing-read' to select one of the addresses."
    (completing-read prompt (cons first addressess)
...
Since we use org and notmuch, makes sense to support linking from one to the other. The =org-notmuch= package is not part of org nor part of notmuch though.
...
A bit unsatisfied with notmuch regarding the compose functions, I've created a =mu4e= config also.
...
#+begin_src emacs-lisp
  (use-package mu4e
    ...
    (setq
     ...
     mu4e-maildir "/media/data/mail"
     mail-user-agent 'mu4e-user-agent
     mu4e-sent-folder "/Sent"
     mu4e-drafts-folder "/Drafts"
     mu4e-trash-folder "/Trash"
     mu4e-refile-folder "/Archives"
     ;; Absolute folder here, not relative to Maildir!
     mu4e-attachment-dir "~/Downloads"

     ;; Added for my specific config
     mrb/mu4e-junk-folder "/Junk"

     mu4e-confirm-quit nil
     mu4e-use-fancy-chars t
     ;; We do mailsyncing somewhere else
     ...
     ;; Read signature from the usual signature file
     mu4e-compose-signature (with-current-buffer
                                (find-file-noselect "~/.signature")
                              (buffer-string))

     ;; Header view settings
     mu4e-headers-include-related t ; Use 'W' to toggle interactively
    ...
    ;; Construct list of my email addresses, the default 'marcel@hsdev.com' is already there
    (add-to-list 'mu4e-user-mail-address-list "mrb@hsdev.com")
    (add-to-list 'mu4e-user-mail-address-list "mvanderboom@gmail.com")
    (add-to-list 'mu4e-user-mail-address-list "marcel@van-der-boom.nl")
    ...
    ;; Let mu4e define a function for it
    (mu4e~headers-defun-mark-for spam)
    (bind-key "S" 'mu4e-headers-mark-for-spam mu4e-headers-mode-map)
    ...
    (setq mu4e-bookmarks (list
                          ...
                          (make-mu4e-bookmark
                           :name "INBOX"
                           :query "(flag:unread or maildir:/INBOX) and not (flag:list or flag:draft or flag:flagged or flag:deleted or maildir:/Trash or flag:trashed)"
                           :key ?i)
                          (make-mu4e-bookmark
                             :name "TODO list"
                             :query "flag:flagged"
...
Not specific to development, but extra useful, is folding of text sections. Origami mode seems to be the generic solution for it. Initially, let's not be too greedy with the keybindings and do it by mode. M-Tab seems to be the natural choice for me.
...
I think most of my documentation needs can be fulfilled with =(helm-)dash=
...
(use-package helm-dash
  ...
  ;; For now we can make all docsets common, i.e. always availabled
  (add-to-list 'helm-dash-common-docsets "Emacs Lisp")
...
#+END_SRC
** Python
Python is an indent aware language, wich saves quite a bit of fiddling around with parenthes and what have you.
