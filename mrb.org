...
- [[https://codeberg.org/mrb/emacs-config][config sources are available on codeberg]]
...
#+begin_src sh :exports code :tangle ~/bin/edit :shebang #!/bin/bash
  ...
  # Ensure availability of commands
  ensure() {
      if ! command -v $1 >/dev/null 2>&1; then
          echo "ERROR: command '$1' is needed"
          exit 1
      fi
...
#+begin_src emacs-lisp
  (use-package stripspace
    ...
    :custom
    ;; We do now want diffs that are to verbose on unclean files
    (stripspace-only-if-initially-clean t)
    ;; make sure we keep in the right position after stripping
  ...
  (use-package autorevert
    ...
    :custom
    (auto-revert-interval 5)               ; Perhaps too often, let's see
...
#+begin_src emacs-lisp
  (use-package pdf-tools
    ...
    :custom
    (pdf-info-epdfinfo-program "/home/mrb/.guix-profile/bin/epdfinfo")
  ...
  (use-package base16-theme
    ...
    (setq base16-adjustments
          `(
            ...
            (font-lock-comment-face         :foreground "bisque4")
...
For file management itself, dired is the defacto standard, but I do not use it much really. The =casual= package is helpfule showing the keyboard bindings.
...
#+begin_src emacs-lisp
  ...
  (use-package org
    ...
    (mrb/org-todo-keyword-CANCELLED((t ( :foreground "bisque4" :weight bold))))
...
#+begin_src emacs-lisp :tangle no
  (setq org-directory "~/dat/org/"
        ...
        ;; NOTE: this will get overridden later on by org-mem
        org-agenda-files (concat org-directory ".agenda_files")
        org-agenda-include-diary t
        org-agenda-start-with-log-mode t
        org-agenda-todo-ignore-scheduled "future"
        org-agenda-ignore-properties '(effort stats appt category)
        org-agenda-todo-ignore-scheduled 'future
...
I combine =ical2orgpy= with a cron job which pulls in the calendars I am interested into. information into the org-mode agenda.
...
#+begin_src emacs-lisp
  (use-package org-journal
    ...
    ;; Put day on top of file, uses `format-time-string`
...
** Dynamic behaviour
Not sure where to put this configuration in general. The problem I'm trying to solve is to keep only relevant files in org-agenda. Too much and the performance collapses, too little and nothing can be found. I'm trying =org-mem= for this, and perhaps add =org-node= later on.

=org-mem= creates a cache of metadata about org-mode files. =org-node= uses this as a library and is similar to =org-roam=, which many people tout as a game changer in todo management, so I think it is worth to dedicate some time to it.

First step is to have org-agenda-files managed automatically based on the cache, filtered for relevance. It does not add much to my current setup, but has the advantage to have this cache available for other purposes (finding things much/much faster is the main thing).

#+begin_src emacs-lisp
  (use-package org-mem
    :after org
    :custom
    ;; Basically list all folders which have org files in them.
    (org-mem-watch-dirs `(,org-directory "~/dat/blogs"))
    (org-mem-do-sync-with-org-id t) ;; not sure what this does exactly
    :config
    ;; Include in agenda-files:
    ;; - non archive named files
    ;; - not my org config file
    ;; - files with active timestamps
    ;; - having entries in /todo/ states
    ;; - having entries scheduled/deadlined
    (defun mrb/set-agenda-files(&rest _)
      (setq org-agenda-files
            (cl-loop
             for file in (org-mem-all-files)
             unless (or (string-search "archive" file)
                        (string-search "org-config.org" file))
             when (seq-find (lambda (entry)
                              (or (org-mem-entry-active-timestamps entry)
                                  (org-mem-entry-todo-state entry)
                                  (org-mem-entry-closed entry)
                                  (org-mem-entry-scheduled entry)
                                  (org-mem-entry-deadline entry)))
                            (org-mem-entries-in file))
             collect file)))
    (add-hook 'org-mem-post-full-scan-functions #'mrb/set-agenda-files)
...
Similarly, the =casual= package shows menu completions, when summoned, for several modes
...
Formatting a file properly is trusted by indent rules, this one indents the whole buffer at once, making formatting completely proper  🫠

#+begin_src elisp
  (defun mrb/indent-whole-buffer ()
  ...
  (global-set-key (kbd "C-c TAB") 'mrb/indent-whole-buffer)
#+end_src
** Navigation
...
#+begin_src emacs-lisp
  ...
  (use-package mastodon
    :straight (mastodon :host codeberg :repo "martianh/mastodon.el"
                        :branch "develop")
    :custom
    (mastodon-instance-url "https://van-der-boom.nl")
    (mastodon-active-user "marcel")
...
Not really man pages as such, but meant as their /advanced/ cousin for GNU programs is the info format.
...
#+begin_src emacs-lisp
  (use-package git-auto-commit-mode
    :demand t
    :hook ((org-journal-mode . git-auto-commit-mode)) ; journals are always autocommitted
