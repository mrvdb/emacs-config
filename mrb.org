...
entry in the archive. I'm not sure if this works out in practice
...
#+BEGIN_SRC emacs-lisp
  (defun mrb/archive-done-item()
    ;; Determine if the item went to the DONE state
    ;; if so, archive it
...
#+BEGIN_SRC emacs-lisp
   ...
   (setq pmpio-auth-token
         [cl-struct-oauth-access-token "ElJu_hKISw0xS2aurzxfeQ" "2mcvN7dVLA-WPqWxbJ5L5sz1YFUWx3uvEMyvPu9-dNU"
                                       [cl-struct-oauth-t "Z96ly0FYf6aOp2_hDiG8_A" "dwriXA736awDlYgoVlQREsBtupkmy1pEqYqNeRX1lHk"]])
   ...
   (defadvice pmpio-ctrl-close-new-note (after delet-pump-frame activate)
     "Advise to close the frame"
