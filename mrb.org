...
#+BEGIN_SRC emacs-lisp
    ...
    (use-package flycheck )      ;; flycheck vs flymake?
...
#+BEGIN_SRC emacs-lisp
  (add-to-list 'load-path "~/dat/src/emacs/packages/sudo-save/")
  (use-package sudo-save :ensure nil)
#+END_SRC
...
(use-package org-capture-pop-frame) ;; Captures in a new frame
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  (defun mrb/make-capture-frame ()
    "Create a new frame for capture purposes."
    (interactive)
