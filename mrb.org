...
#+BEGIN_SRC emacs-lisp
  ...
  (setq default-frame-alist
        '((cursor-type . (bar . 1))
          (cursor-color . "springgreen") ;; TODO: this should be redundant (.Xresources has it)
          (height . 60)
          (width . 100)
  ))
...
#+BEGIN_SRC emacs-lisp
  (use-package flycheck
    ...
    ;; Enable it everywhere until further notice
...
#+BEGIN_SRC emacs-lisp
  (use-package org
    :load-path "~/dat/src/emacs/packages/org-mode/lisp/"
    :ensure nil
    :pin org
    :mode ("\\.txt\\'" "\\.org\'")
    :config
...
(use-package org-present)           ;; Make presentations in org format
...
The =org-capture-pop-frame= makes sure all captures are in separate
frames.
...
#+BEGIN_SRC emacs-lisp
  (use-package org-expiry
    :ensure nil
    :load-path "~/dat/src/emacs/packages/org-mode/contrib/lisp/"
...
#+BEGIN_SRC emacs-lisp
  ...
  ;; FIXME: this causes the TODO capture to resize, which may be normal?
...
#+BEGIN_SRC emacs-lisp
  ...
  ;; this overrules the priority commands, so do this on org-load hook
  (add-hook
   'org-load-hook
   (lambda ()
...
#+BEGIN_SRC emacs-lisp
  (use-package keywiz
    :load-path (lambda () (list (custom-package-location  "keywiz/")))
    :ensure nil)
...
#+BEGIN_SRC emacs-lisp
  (use-package org-journal
    ...
    (progn
      ...
      (setq org-agenda-file-regexp "^[0-9]+")
...
#+BEGIN_SRC emacs-lisp
  ;; Files opened from the tmp folder of claws are messages
  (pushnew '("/home/mrb/.claws-mail/tmp/" . message-mode) auto-mode-alist)
...
#+BEGIN_SRC emacs-lisp
    ...
    (setq group-name-map '
          (("nnvirtual:Discussions" . "INBOX+Sent")
           ("nntp+news.gmane.org:gmane.emacs.devel" . "gmane.emacs.devel")
           ("nntp+news.gmane.org:gmane.emacs.orgmode" . "gmane.emacs.orgmode")
           ("nntp+news.gmane.org:gmane.mail.sylpheed.claws.general" . "gmane.mail.sylpheed.claws.general")
           ("nntp+news.ziggozakelijk.nl:gnu.emacs.gnus" . "gnu.emacs.gnus")))
...
There is a nnreddit backedn I'd like to play with.
#+BEGIN_SRC emacs-lisp
...
#+BEGIN_SRC elisp
   ;;(use-package notmuch) ;; needed? or auto?
...
#+BEGIN_SRC emacs-lisp
  (use-package pumpio-interface
    :load-path "~/dat/src/pump.io.related/pumpio-el/src/"
    :ensure oauth
    :config
    ;; Configure for qua.name
    (setq pumpio-pod "https://status.qua.name")
  )
...
#+BEGIN_SRC emacs-lisp
  (use-package jabber
    :ensure nil
    :load-path "~/dat/src/emacs/packages/jabber.el/"
    :init
    ;; My accounts
    ...
    ;; C-j is the prefix for all jabber command in the C-x map (so,
    ;; C-x C-j precede all commands for jabber) The default C-x map
    ;; for emacs has a C-j entry which binds it to dired-jump. This
    ;; gets in the way of all the keyboard shortcuts for jabber, so
    ;; lets re-call the definition here, so we are sure we get them.
    :bind
    (("C-c C-SPC" . jabber-activity-switch-to)
     :map jabber-chat-mode-map
     ("<tab>" . company-complete)
     :map ctl-x-map
     ("C-j" . jabber-global-keymap)
     :map jabber-global-keymap
     ("C-j" . jabber-muc-join)))
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  (use-package vlc :ensure nil)  ;; Needs my little hack from https://github.com/mrvrb/vlc.el
  ;(use-package async) ;; This is for elfeed, why not add it as dependency there?
  (use-package elfeed
    :init
    (progn
...
#+END_SRC
** IRC
The IRC over XMPP doesn't work at the moment, so I need me some ERC
again.
