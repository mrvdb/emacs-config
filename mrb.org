...
* Emacs configuration                                              [1/92]                      :emacs:exclude:
:PROPERTIES:
:CATEGORY: Emacs
:END:
...
:END:
** TODO a working fill-column indicator which does not kill performance
:PROPERTIES:
:CREATED:  [2015-05-08 vr 13:42]
:END:
** TODO find some specific changes to increase emacs performance, it can be slow at times
:PROPERTIES:
:CREATED:  [2015-05-08 vr 13:42]
:END:
** TODO doc-view window fitting zoom
:PROPERTIES:
:CREATED:  [2015-05-08 vr 13:42]
:END:
** TODO if i had a web browser in an emacs window, i'd get rid of another application
:PROPERTIES:
:CREATED:  [2015-05-08 vr 13:42]
:END:
** TODO threading / multitasking / reliable async
:PROPERTIES:
:CREATED:  [2015-05-08 vr 13:43]
:END:
** TODO much better integration of emacs into social systems
:PROPERTIES:
:CREATED:  [2015-05-08 vr 13:43]
:END:
...
regular expression systax
*** CANCELLED Define specific excercises                                                             :build:
CLOSED: [2015-03-23 ma 11:27]
:LOGBOOK:
- State "CANCELLED"  from "TODO"       [2015-03-23 ma 11:27] \\
  I'm using emacs all day long, the tasks which I create for myself are enough
:END:
:PROPERTIES:
:CREATED: [2012-01-12 do 13:13]
:END:
*** TODO Integrate them into workflow (like defining keyboard shortcuts for them)
:PROPERTIES:
:END:
** TODO Look into the =Effort_ALL= property so I can sort/filter based on estimated effort            :check:
:PROPERTIES:
:CREATED: [2012-06-09 za 15:19]
:END:
See also: http://ur1.ca/9gxhw
** TODO Find out if I can hide the group bots in emacs jabber                                         :check:
:PROPERTIES:
:CREATED: [2012-06-27 wo 10:19]
:END:
** TODO Formatting of list items in emacs epub manual are often split over two lines              :fix:emacs:
** TODO Create a piece of lisp which inserts an in-buffer mode spec line                              :build:
:PROPERTIES:
:CREATED: [2012-07-11 wo 19:56]
:END:
Usecase: files which are not directly associated with a mode, but are
editted frequently.
...
:END:
** TODO Learn about TAGS, seems to make a big difference in editting experience
:PROPERTIES:
:CREATED:  [2013-02-07 do 09:07]
:END:
...
** TODO Make git work in eshell
:PROPERTIES:
:CREATED:  [2013-03-04 ma 15:11]
:END:
Some commands do work properly, but some don't
These do not work:
- git help <something> (git help itself *does* work)

Less relevant the more I use magit
** TODO Configure some tag system for emacs to ease source navigation                                 :emacs:
:PROPERTIES:
:CREATED:  [2013-03-13 wo 09:40]
:END:
At least for odoo.
...
Requirements:
- 1 system for many languages
- emacs support

Preliminary choices:
Seems that a combination of global, exuberant ctags and cscope gives
the most flexible solution.

- [2013-03-13 wo] Installed exuberant-ctags (had to rename the
  ctags/etags installed by emacs itself)
- [2013-03-13 wo] Installed global from the ubuntu repository (not
  sure if this has been compiled with exuberant support, if not, I'll
  need to recompile from source with configure flag)
- [2013-03-13 wo] Tested ctags (M-.) briefly from within emacs, not
  very useful so far, as it often sends to the wrong location
** TODO Learn to use paredit in elisp mode                                                            :check:
:PROPERTIES:
:CREATED:  [2013-03-22 vr 17:46]
:END:
This is probaly something for the longer term. On adding new lisp code
I've got it mastered as long as I can enter code sequentially. The
added value is then guaranteed tree balance and significant fast
editting (the parentheses) slow down a lot.

There are at least 8 or 10 commands to learn for navigating,
inserting, (re-)moving parts of a lisp tree which will take the most
effort.
Long term stuff, but worth the effort I guess.

Not sure if I should reformulate the task to configure paredit and
leave the learning as something ongoing, or leave this task as ongoing
reminder.
...
** TODO Make capturing a link automatic                                                               :build:
:PROPERTIES:
:CREATED:  [2013-06-12 wo 12:20]
:END:
That is, always grab the front link of the browser ready to yank into
emacs
Use org-protocol?

*Always* sounds a bit too intrusive though. There is nothing wrong
 with an explicit action for capturing a link.

Capturing a link and having it published on a shared filesystem
somewhere almost sounds like a 'Pocket replacement'.
...
:END:
** TODO Configure folding for python in emacs
:PROPERTIES:
:CREATED:  [2013-08-01 do 18:19]
:END:
http://www.emacswiki.org/emacs/HideShow seems the easiest option to
go for.

This is just a matter of invoking hs-minor-mode.

The drawback is that the keyboard shortcuts are really bad and it's
different from org-mode where things just seem to work.

These problems are both addressed with:
https://github.com/shanecelis/hideshow-org/blob/master/hideshow-org.el

In python this interferes with something else, so a bit more work is
needed. Even when run manually the hs-org/hideshow gives errors.
...
** TODO Configure a cross-reference system for source code                                      :build:emacs:
:PROPERTIES:
:CREATED:  [2013-10-15 di 15:22]
:END:
- should work with emacs obviously
- languages: C, elisp, python, php at least
- both search caller and callee is neede
- preferably transparent, that is, without a need to go to the
  operating system, just maintenance in emacs itself.

Candidates: etags, ctags, global, exuberant ctags

*** TODO Write a function which transparantly creates tags files when find-tag needs one       :build:emacs:
:PROPERTIES:
:CREATED:  [2013-10-18 vr 21:50]
:END:
- probably set some ground rules on what the location for the file
  needs to be (repetatively go up directories to find the root
  location, or ask?)
- there are several systems, dont really care which one I will use as
  long as the interface is constistent with emacs documentation.
...
** TODO Figure out how to get the erc room topic to render properly if it is long                       :fix:
:PROPERTIES:
:CREATED:  [2013-04-17 wo 20:46]
:END:
...
:END:
** TODO Set up completion in python with ropemacs                                                     :build:
:PROPERTIES:
:CREATED:  [2014-04-09 wo 15:50]
:END:
...
time.
** TODO Add delete-process interactive somewhere
:PROPERTIES:
:CREATED:  [2014-06-11 wo 20:55]
:END:

This is needed more often than not
#+BEGIN_SRC emacs-lisp :tangle no
...
** TODO Review personal keybindings                                                                   :emacs:
:PROPERTIES:
:CREATED:  [2014-06-20 vr 11:33]
:END:
See if we can bring some consistency into it.
[2014-12-24 wo] Printed a copy for review, beter oversight then just
on screen.
** TODO Explore the golden ratio mode                                                           :check:emacs:
:PROPERTIES:
:CREATED:  [2014-11-06 do 16:09]
:END:
Divides emacs frame into uneven sized windows, depending upon which
one has the focus.
** TODO Revisit keychords, the idea still seems nice.                                           :check:emacs:
:PROPERTIES:
:CREATED:  [2014-11-06 do 16:50]
:END:
** TODO Study ibuffer                                                                                 :check:
:PROPERTIES:
:CREATED:  [2014-11-06 do 17:30]
:END:
This seems to be a nice way to manage buffers, like in a project
manager, but indpendent and faster and more integrated in emacs.
...
** TODO Implement a travis config on one of my repositories                                     :build:emacs:
:PROPERTIES:
:CREATED:  [2014-12-23 di 17:56]
:END:
Preferably on an odoo module.
** TODO Fix org-jsinfo message on startup of emacs                                                :emacs:fix:
:PROPERTIES:
:CREATED:  [2014-12-22 ma 12:07]
:END:
...
- custom solution
** TODO Blockdiag looks like something org-mode could benefit from                              :check:emacs:
:PROPERTIES:
:CREATED:  [2014-12-03 wo 11:21]
:END:
Especially the netdiag and rackdiag stuff could be useful to document
our network in a way worthy
** TODO Localisation of org-mode export does not work                                             :fix:emacs:
:PROPERTIES:
:CREATED:  [2014-11-30 zo 17:20]
:END:
things like 'figure' and 'contents' stay untranslate
...
Bind a key to this.
** TODO eshell shortcut does not work in orgmode                                                  :fix:emacs:
:PROPERTIES:
:CREATED:  [2014-11-16 zo 15:52]
:END:
As i am often in orgmode, this is inconvenient
** TODO Creating a new heading with C-Return does not position it correctly on collapsed sections       :fix:
:PROPERTIES:
:CREATED:  [2014-11-11 di 13:00]
:END:
Not sure what the setting should be, but the obvious solution would be
to rebind C-Return.
It only seems to happen when the section after which the new heading
should appear is collapsed. Once the section is expanded, the C-Return
...
be expanded.
** TODO Consider lentic for use with source + orgmode (=literal programming)                          :emacs:
:PROPERTIES:
:CREATED:  [2015-01-09 vr 08:35]
:END:
...
** INFO Multiple file org-mode strategy
Up till now I have mostly managed my information in one org-mode
file. (GTD.org). This has proven to be a fruitful strategy to get
started but over time this just gets a bit cumbersome for a number of
reasons:
- it seems larger files have a major impact on the speed of org-mode
- all information is in one file which makes it harder to focus


I would like to work out a structured way to work in multiple files,
although in part I am obviously already doing this. (Journal is in
journal.org, configuration of orgmode is in a separate file etc.)

I suspect analyzing the GTD file might reveal a suitable split up
strategy. I have already parted some of the projects in separate
files, but this just meant the files and thus the tasks in there got
ignored, so there must be another way to do that so they do not get
ignored.

The part of the system that works really well is the capturing part,
both for TODO items and journalling items.

The part of the system that works least of all is proper organisation
(and likely the discipline to review the tasks properly). I regularly
end up with a whole slew of tasks which I have forgotten or are ill
formed so they become non actionable.

It is probably possible to identify a couple of things I will
need. The capturing system, journalling, blogging and project files
may be part of the things needed. If a project could be the same as
the umbrella, that would simplify things.

Two main things are missing in the system:

1. Mobile
2. Integration with other systems, either ODOO or some other task
   management system (preferably ODOO though)

Integration sounds nice (github and trello come to mind) but that has
not worked in practice for me yet.
Also, the interaction with mail is probably an importatnt part of what
is needed, because many things come in over mail and lead to
tasks. For some of the tasks, this integration is there by forwarding
to tasks@hsdev.com (which creates a new task in ODOO)
** INFO Reading email inside emacs
    ...
    (setq mrb/packages
          (append
           ...
           '(smex ace-jump-mode apache-mode gnuplot-mode sudo-save
                  ...
                  smart-tab yaml-mode hideshow-org oauth flim haskell-mode ghc-mod s f
                  ...
                  git-timemachine eimp mu4e litable diminish paredit fullframe wanderlust)
