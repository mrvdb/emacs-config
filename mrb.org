...
#+BEGIN_SRC emacs-lisp
  (require 'company)
  (defun mrb/indent-or-complete ()
    (interactive)
    (if (minibufferp)
        ;; Just do the normal thing in the minibuffer
        (minibuffer-complete)
      (if (looking-at "\\_>") ;; end of symbol match
          (company-complete-common)
        (indent-according-to-mode))))

  ;; bind tab key this in company mode (which is everywhere by default)
  (define-key company-mode-map "\t" 'mrb/indent-or-complete)
#+END_SRC
