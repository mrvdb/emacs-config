...
Last update: <2021-05-20 11:23:16>
...
#+begin_src emacs-lisp
  ...
  (use-package mu4e
    :ensure nil                           ; dont load from remotes
    ...
    ;;   :ensure nil
    ;;   :load-path (lambda () (concat custom-package-directory "mu4e-thread-folding"))
    ;;   :init
    ;;   ;; Make sure we have our custom emptyr field to put the symbols in place
    ...
    (use-package mu4e-patch
      :ensure nil
      :load-path "~/dat/src/emacs/packages/mu4e-patch"
      :config
    ...
    (add-to-list 'mu4e-view-actions
                 '("b - view in external browser" . mu4e-views-mu4e-view-in-browser-action))
...
Within =mu4e= both gnus and the internal mu4e viewer are supported for rendering messages. I use both, but it's a bit hard to switch between them. As it happens, there is a package to deal with this; =mu4e-views=, so let's use that.

#+begin_src emacs-lisp
  (use-package mu4e-views
    :after mu4e
    :bind (:map mu4e-headers-mode-map
                ;; Main method I want to switch easily
    ...
    (setq mu4e-views-completion-method 'helm
          mu4e-views-default-view-method "text"      ;; set the default, and select it
          mu4e-views-mu4e-use-view-msg-method "text"
          ;; when presssing n and p stay in the current window, not sure if I want this
