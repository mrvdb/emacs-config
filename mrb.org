...
#+BEGIN_SRC emacs-lisp
  ...
  ;; Some leftovers I need to move to a more logical place
  ...
  (use-package fixed-point-completion
    :load-path (lambda () (concat custom-package-directory "fixed-point-completion/")))
#+END_SRC
...
I'm moving to the [[https://github.com/arcticicestudio/nord][Nord color palette]]. This palette defines 16 colors.
...
#+BEGIN_SRC emacs-lisp
   (use-package base16-theme
     ...
     ;; Define a map of facenames/keywords to theme-colorname
     (setq mrb/theme-color-map
           '(("button"                       . :base0C)
             ("show-paren-match-bg"          . :base01)
             ("region-bg"                    . :base03)
             ("default-cursor"               . :base0B)
             ("magit-diff-added-highlight"   . :base0E)
             ("magit-diff-removed-highlight" . :base0B)
             ("magit-diff-added"             . :base0E)
             ("magit-diff-removed"           . :base0B)
             ("org-headline-done"            . :base03)
             ("TODO"                         . :base08)
             ("DONE"                         . :base0E)
             ("WAITING"                      . :base0B)
             ("CANCELLED"                    . :base01)
             ...
             ("READ"                         . :base0F)
             ("PLAN"                         . :base04)
             ("elfeed-search-tag-face"       . :base0E)
             ("elfeed-search-feed-face"      . :base0D)
             ("elfeed-search-date-face"      . :base0A)))

     ;; And a function to get to them
     (defun mrb/themecolor(key)
       ;; returns the color within the theme for faces which are not yet
       ;; defined in it.
       ;; The colors in the theme are defined 'base00` to `base0F`
       (interactive)
       (plist-get mrb/base16-colors
                  (cdr (assoc key mrb/theme-color-map)))))
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  ...
  (set-face-attribute 'button  nil :foreground (mrb/themecolor "button") :weight 'semi-bold)

  ;; Headline for done states less noticeable
  (set-face-attribute 'org-headline-done nil :foreground (mrb/themecolor "org-headline-done"))
...
#+BEGIN_SRC emacs-lisp
  (use-package highlight-parentheses
    ...
    (setq hl-paren-colors
          `(,(plist-get mrb/base16-colors :base0B)
            ,(plist-get mrb/base16-colors :base0E)
            ,(plist-get mrb/base16-colors :base0C)
            ,(plist-get mrb/base16-colors :base0F)
            ,(plist-get mrb/base16-colors :base0D)
            ,(plist-get mrb/base16-colors :base0A)))
    :config
  ...
  (setq mrb-cursor-color (mrb/themecolor "default-cursor"))

  ;; Default frame properties frame position, color, etc
  (setq default-frame-alist
        `((cursor-type . (bar . 1))
          (cursor-color . ,(mrb/themecolor "default-cursor")) ;; TODO: this should be redundant (.Xresources has it)
          (height . 60)
          (width . 100)
  ))

  (use-package cursor-chg
    :load-path (lambda () (concat custom-package-directory "cursor-chg/"))
    :config
    (change-cursor-mode 0)        ; On for overwrite/read-only/input mode
    (toggle-cursor-type-when-idle 1)   ; On when idle
    (setq curchg-default-cursor-color (mrb/themecolor "default-cursor"))
...
#+BEGIN_SRC emacs-lisp
  (defun mrb/run-client-settings()
    ...
    (set-face-attribute 'show-paren-match nil
                        ...
                        :weight 'normal :background (mrb/themecolor "show-paren-match-bg"))

    ;; Set regions so we keep the normal font locking
    (set-face-attribute 'region nil
                        :inherit nil :foreground nil
                        :weight 'normal :background (mrb/themecolor "region-bg"))
...
#+BEGIN_SRC emacs-lisp
  ;; FIXME: change to themecolors
  (setq org-todo-keyword-faces `(
    ("TODO"      . (:foreground ,(mrb/themecolor "TODO") :weight bold))
    ("DONE"      . (:foreground ,(mrb/themecolor "DONE") :weight bold))
    ("WAITING"   . (:foreground ,(mrb/themecolor "WAITING") :weight bold))
    ("CANCELLED" . (:foreground ,(mrb/themecolor "CANCELLED")    :weight bold))
    ("BUY"       . (:foreground ,(mrb/themecolor "BUY")   :weight bold))
    ("HOWTO"     . (:foreground ,(mrb/themecolor "HOWTO")    :weight bold))
    ("INFO"      . (:foreground ,(mrb/themecolor "INFO")       :weight bold))
    ("COLLECT"   . (:foreground ,(mrb/themecolor "COLLECT")   :weight bold))
    ("SOLVE"     . (:foreground ,(mrb/themecolor "SOLVE")     :weight bold))
    ("READ"      . (:foreground ,(mrb/themecolor "READ")  :weight bold))
    ("PLAN"      . (:foreground ,(mrb/themecolor "PLAN")  :weight bold))
  ))
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  (use-package avy
    ...
    (set-face-attribute 'avy-lead-face nil
                        :inherit isearch-lazy-highlight :background "gold" :foreground "black")
    (set-face-attribute 'avy-lead-face-0 nil
                        :inherit isearch-lazy-highlight :background "gold" :foreground "black")
    (set-face-attribute 'avy-background-face nil
                        :inherit isearch-lazy-highlight :background "black" :foreground nil)
    :bind
...
Entering information in text areas in the browser can be delegated to
emacs with the 'atomic-chrome' package. An extension in the browser
sends text into emacs where we can edit as usual. I don't think
embedding emacs in the textarea itself is supported which would be
even nicer (no context switch)
Depending on the url where the text comes from, we can have the buffer
switch to a certain mode; the default mode is text-mode.
...
#+BEGIN_SRC emacs-lisp
  (use-package message
    :ensure nil  ;; Needed because it's internal??
    :requires org-faces
    :config
    ;; First quote level
...
#+BEGIN_SRC emacs-lisp
  (defun mrb/message-mode-hook ()
    ...
    ;; Set the fill column for displaying to window width?
...
#+BEGIN_SRC emacs-lisp
  (use-package elfeed
    ...
    :config
    ;; Adjust faces to themes
    (set-face-attribute 'elfeed-search-tag-face nil :foreground (mrb/themecolor "elfeed-search-tag-face"))
    (set-face-attribute 'elfeed-search-feed-face nil :foreground (mrb/themecolor "elfeed-search-feed-face"))
    (set-face-attribute 'elfeed-search-date-face nil :foreground (mrb/themecolor "elfeed-search-date-face"))
    ...
    (defun mrb/elfeed-add-to-playlist ()
      ...
      (notifications-notify
       :title "Elfeed action"
       :body "Enqueued video in MPV playlist"
       :app-name "Elfeed")
  ...
  (use-package magit
    ...
    (progn
      ...
      (set-face-attribute 'magit-diff-added-highlight nil
                          ...
                          :background (mrb/themecolor "magit-diff-added-highlight"))
      (set-face-attribute 'magit-diff-removed-highlight nil
                          :foreground nil :inherit nil
                          :background (mrb/themecolor "magit-diff-removed-highlight"))
      (set-face-attribute 'magit-diff-added nil
                          :foreground nil :inherit nil
                          :background (mrb/themecolor "magit-diff-added"))
      (set-face-attribute 'magit-diff-removed nil
                          :foreground nil :inherit nil
                          :background (mrb/themecolor "magit-diff-removed"))
