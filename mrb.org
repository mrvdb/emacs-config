...
* Emacs configuration                                              [0/83]                      :emacs:exclude:
:PROPERTIES:
:CATEGORY: Emacs
:END:
...
newline.
** TODO Make inventory for emacs 25 migration
:PROPERTIES:
:CREATED:  [2015-12-06 zo 16:55]
:END:
- font in elfeed is different
- most obbious things seem to work fine
-
...
Questions:
- how would I see what the active window is?
-
...
** TODO Look at shackle for emacs window management                                                    :read:
:PROPERTIES:
:CREATED:  [2015-09-09 wo 10:21]
:END:
https://github.com/wasamasa/shackle

It provides rules based functionality for controlling 'popup' windows
in Emacs. Based on regualar expression matching the size and position
of new windows (buffers) which appear can be controlled.

Example:
Help windows always appear on the bottom, with a height of 30 lines.

It's a nice extension, not sure I want to configure this at this time
until I gather a number of usecases.

Usecases:
- help windows always at the same location, relative to the
  'requesting' buffer, so the information is visible. When closing the
  help window, the original window arrangement should be restored
- magit commit window on top half with the diff in lower half
- customize screens split horizontally
- the org agenda is auto-adjusting to its contents, and shows on the
  bottom
-
...
3.
** TODO Document emacs tip somewhere                                                                  :write:
:PROPERTIES:
:CREATED:  [2015-12-03 do 11:39]
:END:
 Emacs tip: If your instance hangs and won't respond to C-g, you
    can use `pkill -SIGUSR2 emacs` to force emacs to stop whatever
    it's doing.
...
#+BEGIN_SRC emacs-lisp
  ...
  (add-hook 'before-make-frame-hook 'mrb/run-client-settings)
...
#+BEGIN_SRC emacs-lisp
  ...
  ;; Cobble up a set of conditions how to display the date
  ...
  ;; UnreadMarker,SecondaryMarker Treegraph
  ;; precisely 30 chars for the name
  ;; precisely 80 chars for the subject (highlight on hover)
  ;; hard tabulate to pos 115
  ;; insert user-date
  (setq gnus-summary-line-format
        (concat "%*%U%R %B %~(max-right 30)~(pad-right 30)n "
                "%(%~(max-right 80)~(pad-right 80)s%) %-115= %&user-date;\n"))

  (defvar *mrb-mails*
    "marcel@hsdev\\.com\\|mrb@hsdev\\.com\\|m@rcel\\.hsdev\\.com")

  (defun gnus-user-format-function-j (headers)
    (let ((to (gnus-extra-header 'To headers)))
      (if (string-match *mrb-mails* to)
          (if (string-match "," to) "~" "»")
          (if (or (string-match *mrb-mails*
                                (gnus-extra-header 'Cc headers))
                  (string-match *mrb-mails*
...
#+BEGIN_SRC emacs-lisp
  ...
  (setq gnus-message-archive-group
        (format-time-string "nnimap+hsd:INBOX.Sent.-%Y"))
