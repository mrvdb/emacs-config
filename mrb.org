  ...
  (use-package base16-theme
    ...
    (setq mrb/theme-color-map
          '(("button"                       . :base0C)
            ...
            ("ido-bg"                       . :base0B)
            ("ido-first-match"              . :base00)))

    ;; And a function to get to them
    (defun mrb/themecolor(key)
      "Get the theme color for `key`, which mab be some string or the name of a face"
      (interactive)
    ...
    (defun mrb/themeface(face &rest args)
      "Map `face` onto the theme rendering. Basically a shorthand for `set-face-attribute`"

      (if (facep face)
          ;; face can be used directly
          (apply 'set-face-attribute face nil :foreground (mrb/themecolor face) args))
      ;; Return the color value in all cases
      (mrb/themecolor face)))
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  ...
  (mrb/themeface 'button :weight 'semi-bold)

  ;; Headline for done states less noticeable
  (mrb/themeface 'org-headline-done)
...
#+BEGIN_SRC emacs-lisp
  ...
  (setq mrb-cursor-color (mrb/themeface "default-cursor"))

  ;; Default frame properties frame position, color, etc
  (setq default-frame-alist
        `((cursor-type . (bar . 1))
          (cursor-color . ,(mrb/themeface "default-cursor")) ;; TODO: this should be redundant (.Xresources has it)
          (height . 60)
          (width . 100)
  ))
  ...
  (use-package cursor-chg
    ...
    (setq curchg-default-cursor-color (mrb/themeface "default-cursor"))
...
#+BEGIN_SRC emacs-lisp
  ...
  (setq org-todo-keyword-faces `(
    ("TODO"      . (:foreground ,(mrb/themeface "TODO") :weight bold))
    ("DONE"      . (:foreground ,(mrb/themeface "DONE") :weight bold))
    ("WAITING"   . (:foreground ,(mrb/themeface "WAITING") :weight bold))
    ("CANCELLED" . (:foreground ,(mrb/themeface "CANCELLED")    :weight bold))
    ("BUY"       . (:foreground ,(mrb/themeface "BUY")   :weight bold))
    ("HOWTO"     . (:foreground ,(mrb/themeface "HOWTO")    :weight bold))
    ("INFO"      . (:foreground ,(mrb/themeface "INFO")       :weight bold))
    ("COLLECT"   . (:foreground ,(mrb/themeface "COLLECT")   :weight bold))
    ("SOLVE"     . (:foreground ,(mrb/themeface "SOLVE")     :weight bold))
    ("READ"      . (:foreground ,(mrb/themeface "READ")  :weight bold))
    ("PLAN"      . (:foreground ,(mrb/themeface "PLAN")  :weight bold))
  ))
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  (use-package ido
    ...
    (setq
     ...
     (mrb/themeface 'ido-first-match :background (mrb/themecolor "ido-bg"))
    (ido-everywhere))
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  (use-package nnir
    :ensure nil) ;; No need to go load it
...
#+BEGIN_SRC emacs-lisp
  (use-package elfeed
    ...
    (mrb/themeface 'elfeed-search-tag-face)
    (mrb/themeface 'elfeed-search-feed-face)
    (mrb/themeface 'elfeed-search-date-face)
